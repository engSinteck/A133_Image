...
int do_bootr(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	__attribute__((unused)) u32 dsp_id = 0;

	run_addr = simple_strtoul(argv[2], NULL, 16);
	dsp_id = simple_strtoul(argv[3], NULL, 16);
#ifdef CONFIG_XTENSE_DSP
	sunxi_dsp_init(run_addr, dsp_id);
#endif
	return  0;
}
...
static char bootr_help_text[] =
	...
	"\t'arg[3]' can be cpu id of the dsp\n"
#endif
