...
#define VCC_PMU_IO_NAME "aldo3_vol"
...
{
	u8 new_reg_value, old_reg_value;
	...
	if ((set_vol > 0) && (p_item->min_vol)) {
		...
		new_reg_value &= ~p_item->cfg_reg_mask;
		...
		if (old_reg_value != new_reg_value) {
			...
			if (strcmp(name, VCC_PMU_IO_NAME) == 0) {
				tick_printf("VCC-PMU-IO: wait stable\n");
				mdelay(50);
				tick_printf("VCC-PMU-IO: wait stable done\n");

				int old_bias = readl(SUNXI_R_PIO_BASE + 0x340);
				int new_bias = old_bias;
				if (set_vol <= 1800)
					new_bias |= (1 << 0);
				else
					new_bias &= ~(1 << 0);
				...
				if (old_bias != new_bias) {
					tick_printf("PIN-PMU-IO: change bias\n");
					writel(new_bias, SUNXI_R_PIO_BASE + 0x340);
					tick_printf("PIN-PMU-IO: change bias done\n");
				}
			}
		}
	}
	...
	if (onoff == 0) {
		new_reg_value &= ~(1 << p_item->ctrl_bit_ofs);
	} else {
		new_reg_value |= (1 << p_item->ctrl_bit_ofs);
	}
