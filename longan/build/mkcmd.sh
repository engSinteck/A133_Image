...
export LICHEE_CONFIG=$LICHEE_TOP_DIR/device/config/chips/a133/configs
...
function mk_printinfo
{
    local val_list=$1
    local cnt=0

    printf "All available $2:\n"
    for val in $val_list; do
        printf "%4d. %s\n" $cnt $val
        let "cnt++"
    done
...
function storage_list_printinfo
{
	echo "$(eval "$(echo "$(ls -f $1-*)" | sed  "s/^/basename /g")")" | awk -F '-' '{ print $NF}'	
}

function storage_list_file
{
	echo "$(eval "$(echo "$(ls -f $1-*)" | sed  "s/^/basename /g")")"
}

function select_storage()
{
    if [ $LICHEE_PLATFORM == "android" ]; then
        local storage_place=$LICHEE_CONFIG/c4/$LICHEE_PLATFORM/sys_partition.fex
        local STORAGE_ENV=$LICHEE_CONFIG/c4/$LICHEE_PLATFORM
    else    if [ $LICHEE_PLATFORM == "linux" ]; then
                local storage_place=$LICHEE_CONFIG/c4/$LICHEE_LINUX_DEV/sys_partition.fex
                local STORAGE_ENV=$LICHEE_CONFIG/c4/$LICHEE_LINUX_DEV
            fi
    fi

    local cnt=0
	if [ -f "$STORAGE_ENV/sys_partition.fex-16G" ]; then
		local val_list=$(storage_list_printinfo $STORAGE_ENV/sys_partition.fex)
		local copy_list=$(storage_list_file $STORAGE_ENV/sys_partition.fex)
	else
        return
	fi

    mk_printinfo "$val_list" "storage"
    read -p "Choice [storage]: " choice

    if [ -z "${choice}" ] || [ -z "${val_list}" ] || [ -z "${copy_list}" ]; then
        printf "don't change sys_partition.fex ...\n"
        return
    fi

    for val in $copy_list; do
        if [ $choice -eq "$cnt" ]; then
			mk_copy "$STORAGE_ENV/$val" "$storage_place"
			#echo 111111111111$STORAGE_ENV/$val
        fi
        let "cnt++"
    done
}

function select_devicetree()
{
    local device_tree_place=$LICHEE_CONFIG/c4/board.dts
    local BOARD_ENV=$LICHEE_CONFIG/c4

	local cnt=0
	local val_list=$(storage_list_printinfo $BOARD_ENV/board.dts)
	local copy_list=$(storage_list_file $BOARD_ENV/board.dts)

    mk_printinfo "$val_list" "lcd"
    read -p "Choice [lcd]: " choice

    if [ -z "${choice}" ] || [ -z "${val_list}" ] || [ -z "${copy_list}" ]; then
        printf "don't change board.dts ...\n"
        return
    fi

    for val in $copy_list; do
        if [ $choice -eq "$cnt" ]; then
            mk_copy "$BOARD_ENV/$val" "$device_tree_place"
        fi
        let "cnt++"
    done
}
...
function mkdts(){
	...
	if [ "x$LICHEE_KERN_VER" != "xlinux-3.4" ]; then
		...
		cp -v $dts_path/.${LICHEE_CHIP}-*.dtb.d.dtc.tmp ${LICHEE_PLAT_OUT}/.board.dtb.d.dtc.tmp 2>/dev/null
		cp -v $dts_path/.${LICHEE_CHIP}-*.dtb.dts.tmp ${LICHEE_PLAT_OUT}/.board.dtb.dts.tmp 2>/dev/null
		cp -v $dts_path/.board.dtb.d.dtc.tmp ${LICHEE_PLAT_OUT} 2>/dev/null
		cp -v $dts_path/.board.dtb.dts.tmp ${LICHEE_PLAT_OUT} 2>/dev/null
		cp -v $dts_path/*.dtb ${LICHEE_PLAT_OUT}/board.dtb
	fi
}
...
function mk_defroot()
{
	...
	fakeroot mkfs.ubifs -m 4096 -e 258048 -c 145 -F -x zlib -r ${ROOTFS} -o ${LICHEE_PLAT_OUT}/rootfs.ubifs
