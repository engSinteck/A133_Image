...
static void sunxi_rx_sync_audiocodec_route_enable(void *data, bool enable)
{
	struct snd_soc_codec *codec = data;
	struct sunxi_codec_info *sunxi_codec = snd_soc_codec_get_drvdata(codec);

	pr_debug("[%s] line:%d enable:%d\n", __func__, __LINE__, enable);
	regmap_update_bits(sunxi_codec->regmap, SUNXI_ADC_FIFOC,
		(1 << ADC_DRQ_EN), (enable << ADC_DRQ_EN));
	regmap_update_bits(sunxi_codec->regmap, SUNXI_ADC_FIFOC,
		(1 << EN_AD), (enable << EN_AD));
}

static void sunxi_rx_sync_audiocodec_rx_enable(void *data, bool enable)
{
	struct snd_soc_codec *codec = data;
	struct sunxi_codec_info *sunxi_codec = snd_soc_codec_get_drvdata(codec);
	...
	pr_debug("[%s] line:%d enable:%d\n", __func__, __LINE__, enable);
	regmap_update_bits(sunxi_codec->regmap, SUNXI_ADC_FIFOC,
		(1 << RX_SYNC_EN), (enable << RX_SYNC_EN));
}
				...
				struct snd_kcontrol *k, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		} else if (hw_cfg->adcdrc_cfg & DAP_HP_EN) {
...
static int sunxi_codec_rx_sync_get_data(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct snd_soc_codec *codec = snd_soc_component_to_codec(component);
	...
	ucontrol->value.integer.value[0] = sunxi_codec->rx_sync_en;
	return 0;
}

static int sunxi_codec_rx_sync_put_data(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct snd_soc_codec *codec = snd_soc_component_to_codec(component);
	...
	sunxi_codec->rx_sync_en = ucontrol->value.integer.value[0];

	snd_soc_update_bits(codec, SUNXI_ADC_FIFOC,
		(0x1 << RX_EN_MUX), (sunxi_codec->rx_sync_en << RX_EN_MUX));
	return 0;
}
...
static void sunxi_codec_init(struct snd_soc_codec *codec)
{
	...
	/* Keep the Pa enable gpio always on to fix the pop noise of the first key tone */
	if (sunxi_codec->spk_config.pa_power_always_on) {
		gpio_set_value(sunxi_codec->spk_config.spk_gpio,
				(sunxi_codec->spk_config.pa_level));
	}

	if (sunxi_codec->rx_sync_en > 0) {
		snd_soc_update_bits(codec, SUNXI_ADC_FIFOC,
			(0x1 << RX_EN_MUX), (0x1 << RX_EN_MUX));
	}
				...
				struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (sunxi_codec->rx_sync_en > 0) {
			sunxi_rx_sync_startup((void *)codec, RX_SYNC_AUDIOCODEC,
				sunxi_rx_sync_audiocodec_route_enable,
				sunxi_rx_sync_audiocodec_rx_enable);
		}
	}
				...
				struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE &&
		...
		sunxi_rx_sync_shutdown(RX_SYNC_AUDIOCODEC);
	}
}
				...
				int cmd, struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			if (sunxi_codec->rx_sync_en)
				sunxi_rx_sync_control(RX_SYNC_AUDIOCODEC, 0);
		}
		break;
	default:
		return -EINVAL;
...
static int sunxi_codec_suspend(struct snd_soc_codec *codec)
{
	...
	if (!IS_ERR_OR_NULL(sunxi_codec->vol_supply.avcc))
		regulator_disable(sunxi_codec->vol_supply.avcc);

	if (!IS_ERR_OR_NULL(sunxi_codec->vol_supply.cpvin))
...
static int sunxi_internal_codec_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(np, "pa_power_always_on", &temp_val);
	if (ret < 0) {
		dev_warn(&pdev->dev, "pa_power_always_on get failed and set it off.\n");
		sunxi_codec->spk_config.pa_power_always_on = 0;
	} else {
		sunxi_codec->spk_config.pa_power_always_on = temp_val;
	}

	pr_info("digital_vol:%d, lineout_vol:%d, mic1gain:%d, mic2gain:%d"
		" pa_msleep:%d, pa_level:%d, pa_power_always_on:%d\n",
		sunxi_codec->digital_vol,
		...
		sunxi_codec->spk_config.pa_level,
		sunxi_codec->spk_config.pa_power_always_on);
	...
	ret = of_property_read_u32(np, "rx_sync_en", &sunxi_codec->rx_sync_en);
	if (ret < 0)
		sunxi_codec->rx_sync_en = 0;
