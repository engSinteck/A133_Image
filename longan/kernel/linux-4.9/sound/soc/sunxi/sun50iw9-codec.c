...
#if 0
static int sunxi_codec_get_ramp_mode(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct snd_soc_codec *codec = snd_soc_component_to_codec(component);
	...
	regmap_read(sunxi_codec->regmap, AC_DAC_REG, &reg_val);

	ucontrol->value.integer.value[0] = ((reg_val & (1 << RAMPEN)) ? 1 : 0);

	return 0;
}

static int sunxi_codec_set_ramp_mode(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct snd_soc_codec *codec = snd_soc_component_to_codec(component);
	...
	case	0:
		...
		regmap_update_bits(sunxi_codec->regmap, AC_RAMP_REG,
				(0x1 << RDEN), (0x0 << RDEN));
		break;
	case	1:
		regmap_update_bits(sunxi_codec->regmap, AC_DAC_REG,
...
/* sunxi codec ramp function choose to use */
static const char * const sunxi_codec_ramp_function[] = {"Disabled", "Enabled"};

static const struct soc_enum sunxi_codec_ramp_func_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(sunxi_codec_ramp_function),
			sunxi_codec_ramp_function),
};
#endif
				  ...
				  struct snd_kcontrol *k, int event)
{
	...
	case	SND_SOC_DAPM_PRE_PMD:
		if (spk_cfg->used) {
			gpio_set_value(spk_cfg->gpio, !spk_cfg->pa_ctl_level);
			mdelay(spk_cfg->pa_msleep_time * 2);
		}
...
#if 0
	SOC_ENUM_EXT("ramp func mode", sunxi_codec_ramp_func_enum[0],
				sunxi_codec_get_ramp_mode,
				sunxi_codec_set_ramp_mode),
#endif
...
static void sunxi_codec_init(struct snd_soc_codec *codec)
{
	...
	regmap_update_bits(sunxi_codec->regmap, SUNXI_DAC_DPC,
			(0x3f << DVOL), (gain_cfg->digital_vol << DVOL));
			...
			int clk_id, unsigned int freq, int dir)
{
	...
	if (clk_set_rate(sunxi_codec->pllclkx4, freq)) {
		pr_err("audiocodec: codec set pllclk rate failed\n");
		return -EINVAL;
	}

	/* moduleclk freq = 49.152/45.1584M, audio clk source = 98.304/90.3168M */
	if (clk_set_rate(sunxi_codec->moduleclk, freq / 2)) {
		pr_err("audiocodec: codec set module clk rate failed\n");
		return -EINVAL;
	}
...
static int sunxi_codec_suspend(struct snd_soc_codec *codec)
{
	...
	if (sunxi_codec->vol_supply.cpvdd)
		regulator_disable(sunxi_codec->vol_supply.cpvdd);
...
static int sunxi_codec_resume(struct snd_soc_codec *codec)
{
	...
	if (sunxi_codec->vol_supply.cpvdd) {
		ret = regulator_enable(sunxi_codec->vol_supply.cpvdd);
		if (ret)
			pr_err("[%s]: avcc:enable() failed!\n", __func__);
	}
...
static int sunxi_internal_codec_probe(struct platform_device *pdev)
{
	...
	sunxi_codec->vol_supply.cpvdd = regulator_get(&pdev->dev,
								"vcc33_audio");
	if (IS_ERR(sunxi_codec->vol_supply.cpvdd)) {
		pr_err("[%s]:get audio vcc3v3-audio failed\n", __func__);
		sunxi_codec->vol_supply.cpvdd = NULL;
	} else {
		ret = regulator_enable(sunxi_codec->vol_supply.cpvdd);
		if (ret) {
			pr_err("[%s]:vcc3v3-audio enable failed!\n", __func__);
			goto err_put_regulator;
		}
	}
	...
	if (IS_ERR_OR_NULL(sunxi_codec->pllclk)) {
		pr_err("audiocodec: pllclk not exist or invaild\n");
	...
	if (IS_ERR_OR_NULL(sunxi_codec->moduleclk)) {
		pr_err("audiocodec: moduleclk not exist or invaild\n");
		ret = PTR_ERR(sunxi_codec->moduleclk);
		goto err_devm_kfree;
	} else {
		if (clk_set_parent(sunxi_codec->moduleclk, sunxi_codec->pllclkx4)) {
			pr_err("audiocodec: set parent of moduleclk to pllclkx4 failed\n");
			ret = -EBUSY;
			goto err_devm_kfree;
		}
	}
	...
	ret = of_property_read_u32(np, "ramp_func_used", &temp_val);
	if (ret < 0) {
		dev_warn(&pdev->dev, "ramp_func_used get failed\n");
		spk_gpio->ramp_func_used = 0;
	} else {
		spk_gpio->ramp_func_used = temp_val;
	}
	...
	pr_err("lineout_vol:%d, linein_gain:%d, fmin_gain:%d, "
		"digital_vol:%d, adcdrc_cfg:%d, adchpf_cfg:%d, "
		"dacdrc_cfg:%d, dachpf_cfg:%d, ramp_func_used:%d, "
		"pa_msleep_time:%d, pa_ctl_level:%d, gpio-spk:%d\n",
		...
		sunxi_codec->hw_cfg.adcdrc_cfg,
		sunxi_codec->hw_cfg.adchpf_cfg,
		sunxi_codec->hw_cfg.dacdrc_cfg,
		sunxi_codec->hw_cfg.dachpf_cfg,
		sunxi_codec->spk_cfg.ramp_func_used,
		sunxi_codec->spk_cfg.pa_msleep_time,
		spk_gpio->pa_ctl_level,
		spk_gpio->gpio);
