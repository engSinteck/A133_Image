...
typedef void (*enable_func)(void *data, bool onoff);
...
static struct {
	rx_sync_dev_t dev_info[RX_SYNC_MAX];
	int startup_count;
	int count;
	int state;
	int obj;
...
{
	if (type < 0 || type >= RX_SYNC_MAX)
		return;
	spin_lock_irq(&rx_sync_lock);
	rx_sync_data.dev_info[type].data = data;
	rx_sync_data.dev_info[type].enable_seq = 0;
	rx_sync_data.dev_info[type].route_enable = route_enable;
	rx_sync_data.dev_info[type].rx_enable = rx_enable;

	rx_sync_data.obj |= (1 << type);
	rx_sync_data.state = RX_SYNC_STATE_IDLE;
	rx_sync_data.startup_count++;
	pr_info("[%s] line:%d type=%d, startup_count=%d\n", __func__, __LINE__, type, rx_sync_data.startup_count);
	spin_unlock_irq(&rx_sync_lock);
}
...
void sunxi_rx_sync_shutdown(int type)
{
	if (type < 0 || type >= RX_SYNC_MAX)
		return;
	spin_lock_irq(&rx_sync_lock);
	pr_info("[%s] line:%d count=%d, startup_count=%d\n", __func__, __LINE__,
				rx_sync_data.count,
				rx_sync_data.startup_count);
	if (rx_sync_data.count == 0) {
		memset(&rx_sync_data, 0, sizeof(rx_sync_data));
	}
	spin_unlock_irq(&rx_sync_lock);
}
EXPORT_SYMBOL(sunxi_rx_sync_shutdown);

static void sunxi_rx_sync_disable(void)
{
	int i;

	pr_info("[%s] line:%d obj:0x%x\n", __func__, __LINE__, rx_sync_data.obj);
	for (i = 0; i < RX_SYNC_MAX; i++) {
		void *data = rx_sync_data.dev_info[i].data;

		if (!(rx_sync_data.obj & (1 << i)))
			continue;
		pr_info("[%s] line:%d type=%d enable_seq=%d, startup_count=%d\n",
					__func__, __LINE__,
					i, rx_sync_data.dev_info[i].enable_seq,
					rx_sync_data.startup_count);
		if (rx_sync_data.dev_info[i].enable_seq !=
			rx_sync_data.startup_count)
			continue;
		pr_info("[%s] line:%d disable rx_sync with type %d\n", __func__, __LINE__, i);
		rx_sync_data.dev_info[i].rx_enable(data, 0);
		break;
	}
}

static void sunxi_rx_sync_route_enable_all(void)
{
	int i, seq = 0;
	...
	for (i = 0; i < RX_SYNC_MAX; i++) {
		...
		if (!(rx_sync_data.obj & (1 << i)))
			continue;
		rx_sync_data.dev_info[i].route_enable(data, 1);
		rx_sync_data.dev_info[i].enable_seq = ++seq;
		pr_info("[%s] line:%d type:%d enable_seq:%d\n", __func__, __LINE__,
			i, rx_sync_data.dev_info[i].enable_seq);
		if (rx_sync_data.startup_count == seq) {
			rx_sync_data.dev_info[i].rx_enable(data, 1);
			rx_sync_data.count = rx_sync_data.startup_count;
			break;
		}
	}
}
...
{
	unsigned long flags;
	pr_info("[%s] line:%d type:%d, enable:%d, count:%d, startup_count:%d\n",
			__func__, __LINE__, type, enable,
		rx_sync_data.count, rx_sync_data.startup_count);
	if (type < 0 || type >= RX_SYNC_MAX)
		return;
	spin_lock_irqsave(&rx_sync_lock, flags);
	if (enable) {
		if (rx_sync_data.count >= rx_sync_data.startup_count) {
			goto unlock;
		}
		rx_sync_data.count++;
		if (rx_sync_data.state == RX_SYNC_STATE_WORK) {
			pr_info("[%s] line:%d route enable all\n", __func__, __LINE__);
			sunxi_rx_sync_route_enable_all();
			goto unlock;
		}
		rx_sync_data.dev_info[type].enable_seq = rx_sync_data.count;
		if (rx_sync_data.count == 2) {
			void *data = rx_sync_data.dev_info[type].data;
			pr_info("[%s] line:%d enable rx with type %d\n", __func__, __LINE__, type);
			rx_sync_data.dev_info[type].rx_enable(data, 1);
			rx_sync_data.state = RX_SYNC_STATE_WORK;
		}
	} else {
		if (rx_sync_data.count == 0) {
			goto unlock;
