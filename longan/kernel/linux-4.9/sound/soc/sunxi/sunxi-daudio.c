...
static int sunxi_daudio_rx_sync_get_data(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct snd_soc_codec *codec = snd_soc_component_to_codec(component);
	...
	ucontrol->value.integer.value[0] = sunxi_daudio->pdata->rx_sync_en;
	return 0;
}

static int sunxi_daudio_rx_sync_put_data(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct snd_soc_codec *codec = snd_soc_component_to_codec(component);
	...
	sunxi_daudio->pdata->rx_sync_en = ucontrol->value.integer.value[0];
	regmap_update_bits(sunxi_daudio->regmap, SUNXI_DAUDIO_CTL,
			(0x1 << RX_EN_MUX),
			(sunxi_daudio->pdata->rx_sync_en << RX_EN_MUX));
	return 0;
}
...
#endif
	SOC_SINGLE_BOOL_EXT("daudio rx_sync switch", 0,
		sunxi_daudio_rx_sync_get_data, sunxi_daudio_rx_sync_put_data),
};
...
static void sunxi_rx_sync_daudio_route_enable(void *data, bool enable)
{
	struct sunxi_daudio_info *sunxi_daudio = data;

	pr_debug("[%s] line:%d enable:%d\n", __func__, __LINE__, enable);
	regmap_update_bits(sunxi_daudio->regmap, SUNXI_DAUDIO_CTL,
			(1<<CTL_RXEN), (enable<<CTL_RXEN));
	regmap_update_bits(sunxi_daudio->regmap, SUNXI_DAUDIO_INTCTL,
			(1<<RXDRQEN), (enable<<RXDRQEN));
}
					...
					int enable)
{
	...
	if (sunxi_daudio->pdata->rx_sync_en) {
		int type = sunxi_daudio->pdata->tdm_num + RX_SYNC_DAUDIO0;
		sunxi_rx_sync_control(type, enable);
	}
}
				...
				struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE &&
		sunxi_daudio->pdata->rx_sync_en > 0) {
		int type = sunxi_daudio->pdata->tdm_num + RX_SYNC_DAUDIO0;
		sunxi_rx_sync_startup((void *)sunxi_daudio, type,
			sunxi_rx_sync_daudio_route_enable,
...
static int sunxi_daudio_dev_probe(struct platform_device *pdev)
{
	...
	case	SUNXI_DAUDIO_EXTERNAL_TYPE:
		...
		if (ret < 0) {
			dev_warn(&pdev->dev, "rx_sync_en missing or invalid\n");
			sunxi_daudio->pdata->rx_sync_en = 0;
		} else {
			sunxi_daudio->pdata->rx_sync_en = temp_val;
		}
