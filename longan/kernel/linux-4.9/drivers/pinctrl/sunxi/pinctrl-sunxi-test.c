...
static int pctrltest_direction_input(void)
{
	...
	if (IS_ERR_VALUE((unsigned long)direct_status)) {
...
static int pctrltest_request_eint(void)
{
	...
	if (IS_ERR_VALUE((unsigned long)virq)) {
...
static int gpiotest_gpiolib(void)
{
	...
	if (IS_ERR_VALUE((unsigned long)set_direct_status)) {
...
static int sunxi_pctrltest_probe(struct platform_device *pdev)
{
	...
	struct gpio_config config, config_suspend;
	int gpio, irq;
	int ret;
	...
	suspend_gpio = of_get_named_gpio_flags(np, "suspend-gpios", 0,
				       (enum of_gpio_flags *)&config_suspend);
	if (!gpio_is_valid(gpio)) {
		pr_err("Vdevice failed to get test-gpios\n");
		return -ENODEV;
	}
	...
	ret = request_irq(gpio_irq, sunxi_pinctrl_irq_handler_demo3,
