...
#define KEY_NUM		8
static const char *goodix_ts_name = "goodix-ts-2";
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
	...
	ts->pen_dev->id.bustype = BUS_USB;  // for external devices
...
static void goodix_ts_suspend(struct goodix_ts_data *ts)
{
	...
	if (!gtp_gesture_wakeup) {
		printk("ctp dup power off suspend!!!\n");
		input_set_power_enable(&(config_info.input_type), 0);
		__gpio_set_value(config_info.wakeup_gpio.gpio, 0);
	}
}
