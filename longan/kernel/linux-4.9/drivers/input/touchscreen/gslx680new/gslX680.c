...
static ssize_t gslX680_dump_show(
	struct device *dev,
	struct device_attribute *attr,
	char *buf)
{
	...
	for (i = 0; i <= 0xff; i++) {
		gsl_ts_read(ts_init->client, i, &mem_buf, sizeof(mem_buf));
...
static DEVICE_ATTR(dump, 0444, gslX680_dump_show, NULL);
...
static unsigned long idle_data = 1;
#ifdef CONFIG_PM
...
static ssize_t gslclass_enable_show(struct class *cls,
...
static ssize_t gslclass_enable_store(struct class *cls,
...
static ssize_t gslclass_idle_enable_show(struct class *cls,
			struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", (int)idle_data);
}
...
static ssize_t gsl_idle_enable_store_l(const char *buf)
{
	...
	error = kstrtoul(buf, 10, &idle_data);
	if (error)
		return error;
...
static ssize_t gslclass_idle_enable_store(struct class *cls,
	       struct class_attribute *attr,
	       const char *buf, size_t count)
{
...
static struct class_attribute ctp_class_attrs[] = {
	__ATTR(runtime_suspend, S_IRUGO | S_IWUSR | S_IWGRP, gslclass_enable_show, gslclass_enable_store),
	__ATTR(tp_idle, S_IRUGO | S_IWUSR | S_IWGRP, gslclass_idle_enable_show, gslclass_idle_enable_store),
	__ATTR_NULL,
};
			...
			const struct i2c_device_id *id)
{
	...
	device_create_file(&ts->input->dev, &dev_attr_dump);
...
static int ctp_get_system_config(void)
{
	...
	if (fw_index == -1) {
		pr_err("gslx680: no matched TP firmware(%s)!\n", fwname);
		return 0;
	}
