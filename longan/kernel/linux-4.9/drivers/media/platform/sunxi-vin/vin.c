...
static int vin_md_get_clocks(struct vin_md *vind)
{
	...
	vind->clk[VIN_TOP_CLK_FREQ].clock = of_clk_get(np, 3 * i + 4);
	if (IS_ERR_OR_NULL(vind->clk[VIN_TOP_CLK_FREQ].clock)) {
		vin_err("Get top frequency clk source failed!\n");
		vind->clk[VIN_TOP_CLK_FREQ].clock = NULL;
		return 0;
	}
	vind->isp_clk[VIN_ISP_CLK_FREQ].clock = of_clk_get(np, 3 * i + 5);
	if (IS_ERR_OR_NULL(vind->clk[VIN_ISP_CLK_FREQ].clock)) {
		vin_err("Get isp frequency clk source failed!\n");
		vind->clk[VIN_ISP_CLK_FREQ].clock = NULL;
		return 0;
	}
...
#if defined CONFIG_ARCH_SUN50IW10P1
	if (clk_set_rate(vind->clk[VIN_TOP_CLK_FREQ].clock, rate)) {
...
#if defined CONFIG_ARCH_SUN50IW10P1
	if (clk_set_rate(vind->isp_clk[VIN_ISP_CLK_FREQ].clock, rate)) {
		vin_err("set vin isp freq clock source rate error\n");
		return -1;
	}
#endif
