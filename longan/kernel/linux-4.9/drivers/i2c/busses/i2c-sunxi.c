...
sunxi_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
	...
	if (ret < 0)
		goto out;
	if (i2c->twi_drv_used) {
		if (!(readl(i2c->base_addr + TWI_DRIVER_CTRL) & TWI_DRV_EN)) {
			I2C_ERR("[i2c%d] has been disabled.\n", i2c->bus_num);
			return -EACCES;
		}
...
static void sunxi_i2c_shutdown(struct platform_device *pdev)
{
	...
	/* only pmu device`s twi has no_suspend and twi_drv_used */
	if ((i2c->no_suspend) && (i2c->twi_drv_used)) {
		while (((readl(i2c->base_addr + TWI_DRIVER_CTRL) &
				TWI_DRV_STA) >> 16) != 0xf8) {
			twi_timeout++;
			mdelay(10);
			if (twi_timeout == 300) {
				I2C_ERR("[i2c%d] status is %x\n", i2c->bus_num,
					readl(i2c->base_addr + TWI_DRIVER_CTRL));
				BUG_ON(1);
			}
		}
	}
