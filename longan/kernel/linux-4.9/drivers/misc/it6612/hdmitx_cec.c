...
CECcmdFormat  LookUpTable_CEC[]=
{ {eFeatureAbort,		eDirectly,	 4},		// [Header] + [0x00] + [Feature OpCode] + [Abort Reason] {eAbort,				   eDirectly,	 2},		// [Header] + [0xFF]
	// One Touch Play Feature {eActiveSource,		eBroadcast, 4},		// Header + [0x82] + [Physical Address] {eImageViewOn,		 eDirectly,	 2},		// Header + [0x04] {eTextViewOn,		  eDirectly,	 2},		// Header + [0x0D]
	// Routing Control Feature {eInactiveSource,		  eBroadcast, 2},		// Header + [0x9D] {eRequestActiveSource,	 eBroadcast, 2},		// Header + [0x85] {eRoutingChange,		   eBroadcast, 6},		// Header + [0x80] + [Org Address] + [New Address] {eRoutingInformation,	  eBroadcast, 4},		// Header + [0x80] + [Org Address] + [New Address] {eSetStreamPath,		   eBroadcast, 4},		// Header + [0x86] + [Physical Address] {eStandBy,			 eBoth,	 2},		// Header + [0x36]
	// System Information Feature {eCECVersioin,		 eDirectly,	 3},		// Header + [0x82] + [CEC Version] {eGetCECVersion,		   eDirectly,	 2},		// Header + [0x9F] {eGivePhysicalAddress,	 eDirectly,	 2},		// Header + [0x83] {eGetMenuLanguage,	 eDirectly,	 2},		// Header + [0x91] {eReportPhysicalAddress,   eBroadcast, 5},		// Header + [0x84] + [Physical Address]+ [Device Type] {eSetMenuLanguage,	 eBroadcast, 3},		// Header + [0x32] + [Language]
	// Vendor Specific Commands Feature {eDeviceVendorID,	  eBroadcast, 5},		// Header + [0x87] + [Vendor ID] {eGiveDeviceVendorID,	  eDirectly,	 2},		// Header + [0x8C] {eVendorCommand,	   eDirectly,	 3},		// Header + [0x89] + [Vendor Specific Data] {eVendorCommandWithID, eBoth,	 6},		// Header + [0xA0] + [Vendor ID]+ [Vendor Specific Data] {eRemoteButtonDown,	eBoth,	 3},		// Header + [0x8A] + [Vendor Specific RC code] {eRemoteButtonUp,	  eBoth,	 3},		// Header + [0x8B] + [Vendor Specific RC code]
	// Remote Control PassThrough Feature {eUserPressed,		 eDirectly,	 3},		// Header + [0x44] + [UI command] {eUserReleased,		eDirectly,	 2},		// Header + [0x45]
	// Power Status Feature {eGiveDevicePowerStatus,   eDirectly,	 2},		// Header + [0x8F] {eReportPowerStatus,	   eDirectly,	 3},		// Header + [0x90] + [Power Status]
	// System Audio Control Feature {eGiveAudioStatus,	 eDirectly,	 2},		// Header + [0x71] {eGiveSystemAudIoModeStatus,eDirectly, 2},		// Header + [0x7D] {eReportAudioStatus,	   eDirectly,	 3},		// Header + [0x7A] + [Audio Status] {eReportAudioDescriptor,   eDirectly,	 4},		// Header + [0xA3] + [Audio Descriptor] {eRequestAudioDescriptor,  eDirectly,	 3},		// Header + [0xA4] + [Audio Format ID and Code] {eSetSystemAudioMode	,  eBoth,	 3},		// Header + [0x72] + [System Audio Status] {eAudioModeRequest,	eDirectly,	 4},		// Header + [0x70] + [Physical Address] {eSystemAudIoModeStatus,   eDirectly,	 3},		// Header + [0x7E] + [System Audio Status]
	// ACR {eInitiateARC,			 eDirectly,	 2},		// Header + [0xC0] {eReportARCInitiated,	  eDirectly,	 2},		// Header + [0xC1] {eReportARCTerminated, eDirectly,	 2},		// Header + [0xC2] {eRequestARCInitiation,	eDirectly,	 2},		// Header + [0xC3] {eRequestARCTermination,   eDirectly,	 2},		// Header + [0xC4] {eTerminateARC,		eDirectly,	 2},		// Header + [0xC5]
	// HEC {eCDC,				 eDirectly,	 3},		// Header + [0xF8]
	// other {ePlay,				eDirectly,	 3},		// Header + [0x41] + [Play Mode]
	// Polling Mesage
	//ePollingMessage, {eCDC+1,			   eBoth,	 0},		//end of	 lookup table !!!
};
...
void cecconfig( BYTE LogicAdr, BYTE TimeUnit, BYTE CECRst, BYTE EnNAck)
{
	...
	cecclrint();
#ifdef DEBUG_iT6601
	IT6601_PRINT(("\nSet Configuration : LogicAdr=%bX, TimeUnit=%bX, CECRst=%bX, EnNAck=%bX, RxSelf_Dis=%bX\n\n", LogicAdr, TimeUnit, CECRst, EnNAck, RxSelf_Dis));
#endif
}
	...
	switch (CECTxState) {
		...
		case sCECTransfer:
		{
				if (cecfirerdy() == TRUE)
				{
					...
					//IT6601_PRINT(("CEC Tx Handler [ %bX ]\n",OldTxCECcmd));
...
#ifdef DEBUG_iT6601
			IT6601_PRINT(("PollingMessage Header=%bX \n",TxCmdBuf.HEADER));
#endif
			TxCmdBuf.SIZE=1;
			break;
...
#ifdef DEBUG_iT6601
	printf("CECCmdSet --> Tx Header:%bX, opcode:%bX , operand1:%bX , operand2:%bX ,Size:%bX-----\n ",
		   TxCmdBuf.HEADER,
		   TxCmdBuf.OPCODE,
		   TxCmdBuf.OPERAND1,
		   TxCmdBuf.OPERAND2,
...
void CECDecoder(void)
{
	...
	switch (pCECRxBuf->SIZE) {
		default:
		case 18:
			IT6601_PRINT(("OPERAND16=%bX \n",pCECRxBuf->OPERAND16));
		case 17:
			IT6601_PRINT(("OPERAND15=%bX \n",pCECRxBuf->OPERAND15));
		case 16:
			IT6601_PRINT(("OPERAND14=%bX \n",pCECRxBuf->OPERAND14));
		case 15:
			IT6601_PRINT(("OPERAND13=%bX \n",pCECRxBuf->OPERAND13));
		case 14:
			IT6601_PRINT(("OPERAND12=%bX \n",pCECRxBuf->OPERAND12));
		case 13:
			IT6601_PRINT(("OPERAND11=%bX \n",pCECRxBuf->OPERAND11));
		case 12:
			IT6601_PRINT(("OPERAND10=%bX \n",pCECRxBuf->OPERAND10));
		case 11:
			IT6601_PRINT(("OPERAND9=%bX \n",pCECRxBuf->OPERAND9));
		case 10:
			IT6601_PRINT(("OPERAND8=%bX \n",pCECRxBuf->OPERAND8));
		case 9:
			IT6601_PRINT(("OPERAND7=%bX \n",pCECRxBuf->OPERAND7));
		case 8:
			IT6601_PRINT(("OPERAND6=%bX \n",pCECRxBuf->OPERAND6));
		case 7:
			IT6601_PRINT(("OPERAND5=%bX \n",pCECRxBuf->OPERAND5));
		case 6:
			IT6601_PRINT(("OPERAND4=%bX \n",pCECRxBuf->OPERAND4));
		case 5:
			IT6601_PRINT(("OPERAND3=%bX \n",pCECRxBuf->OPERAND3));
		case 4:
			IT6601_PRINT(("OPERAND2=%bX \n",pCECRxBuf->OPERAND2));
		case 3:
			IT6601_PRINT(("OPERAND1=%bX \n",pCECRxBuf->OPERAND1));
		case 2:
			IT6601_PRINT(("OPCODE=%bX \n",pCECRxBuf->OPCODE));
		case 1:
			IT6601_PRINT(("HEADER=%bX \n",pCECRxBuf->HEADER));
		case 0:
			IT6601_PRINT(("SIZE =%bX \n",pCECRxBuf->SIZE));
			break;
	}
#endif

	FollowerAddr = (pCECRxBuf->HEADER & 0x0F);	// Get Follower logic address ;
	IT6601_PRINT(("FollowerAddr=%bX \n", FollowerAddr));
	if (pCECRxBuf->SIZE == 1) {
	...
	IT6601_PRINT(("SizeOfLookUpTable_CEC=%bX \n", index));
		...
		case eInactiveSource :	  //<MS> <0x9D> <Physical Address>		example:<0x40> <0x9D> <AB> <CD>
			...
			if (Myself_LogicAdr == DEVICE_ID_TV)   // for TV only !!!
			{
				...
				//f(Myself_LogicAdr==uc)
				{
					...
					{
						IT6601_PRINT(("CECList[%bX].Active -> FALSE\n",uc));
...
//=== 13.6 System Information ===
		case eCECVersioin :	 //<MS> <0x9E> <CEC version>
			IT6601_PRINT(("Receive !!!CEC Version =%bX!!!\n",pCECRxBuf->OPERAND1));
			break;
...
#ifdef _MCU_8051_
			P1_6=1;
			CECCmdSet(uc, eCECVersioin,((P1_6 == 1)?0x04:0x5),0); // 5 for Version 1.4a... 4 for Version 1.3A...0~3 for Reserved
#endif
			break;
...
//			if(CheckUpDownPhyAdr()==TRUE)
		...
		case eReportARCInitiated :
			printf("Receive !!!Report ARC Initiated !!!\n");
			break;
		case eReportARCTerminated :
			printf("Receive !!!Report ARC Terminated !!!\n");
...
//			DumpiT6601Reg();
		...
		case eRequestARCInitiation :
			printf("Receive !!!Request ARC Initiation !!!\n");
//			if(CheckUpDownPhyAdr()==TRUE)
			{
		...
		case eRequestARCTermination :	//Request ARC Termination
			printf("Receive !!!Request ARC Termination !!!\n");
			if (CheckUpDownPhyAdr() == TRUE)
			{
...
{
	IT6601_PRINT(("Switch_OneTouchPlayState--> %bX \n",uState));
...
{
		IT6601_PRINT(("switch_PollingMessage--> %bX \n",state));
		switch (state)
		{
...
// for iT6633+iT6623{
// for iT6633+iT6623	IT6601_PRINT(("Switch_LogicalAddressingState--> %bX \n",uState));
		...
		case sPollingMessage:
			{
				...
				else
				{
					...
					for (uc=1;uc<15;uc++)
					{
						IT6601_PRINT(("CECList[%bX]= %bX , PhysicalAddr=[%bX][%bX] \n",
									  uc,
									  CECList[uc].Active,
		...
		case sReportPhysicalAddressReceived:
			{
				...
				if (pCECRxBuf->OPCODE == eReportPhysicalAddress)
				{
					...
					IT6601_PRINT(("-----Rx Header=%bX, opcode=%bX , operand1=%bX , operand2=%bX ,cPollingCount=%bX-----\n ",
								  pCECRxBuf->HEADER,
								  pCECRxBuf->OPCODE,
								  pCECRxBuf->OPERAND1,
								  pCECRxBuf->OPERAND2,
...
BOOL CheckUpDownPhyAdr(void)
{
	u8_t i;
	BOOL Result=FALSE;
	BYTE pA,pB,pC,pD,MyLevel = 0xFF,TargetLevel = 0xFF;
	for (i=0;i<4;i++) {
		...
		case	sHEC_InquirState:
			{
				...
				if (pCECRxBuf->OPERAND3 == eHEC_InquirState)
				{
					...
					//FollowerAddr=uc;
					//pCECRxBuf->OPERAND1
					IT6601_PRINT(("-----Header=%bX, opcode=%bX , operand1=%bX , operand2=%bX ,FollowerAddr=%bX-----\n ",
								  pCECRxBuf->HEADER,
								  pCECRxBuf->OPCODE,
								  pCECRxBuf->OPERAND1,
								  pCECRxBuf->OPERAND2,
...
// for iT6633+iT6623			IT6601_PRINT((" == =CEC Active Port = %bX  == =\n",i));
// for iT6633+iT6623
...
{
	IT6601_PRINT(("!!! SwitchSystemInfoFeature -> %bX !!!\n",State));
	SACState=State;
	switch (SACState) {
...
// for iT6633+iT6623						printf("CEC volume = %bX \n",uCECAudioStatus.Map.AudioVolumeStatus);
			}
			else
			{
				if ((uCECAudioStatus.Map.AudioVolumeStatus) == 0x00)
