...
#if defined(CONFIG_ARCH_SUN50IW10)
	if (sunxi_ohci->usbc_no == HCI0_USBC_NO) {
		val = readl(sunxi_ohci->usb_ccmu_config + USB_GATE);
		val |= (SUNXI_CCMU_USBEHCI1_GATING_OFFSET | SUNXI_CCMU_USBEHCI1_RST_OFFSET);
		writel(val, sunxi_ohci->usb_ccmu_config + USB_GATE);

		val = readl(sunxi_ohci->usb_ccmu_config + USB1_CFG);
		val |= (SUNXI_CCMU_SCLK_GATING_USBPHY1_OFFSET
			| SUNXI_CCMU_USBPHY1_RST_OFFSET
			| SUNXI_CCMU_SCLK_GATING_OHCI1_OFFSET);
		writel(val, sunxi_ohci->usb_ccmu_config + USB1_CFG);
