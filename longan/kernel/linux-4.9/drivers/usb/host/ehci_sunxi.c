...
#if defined(CONFIG_ARCH_SUN50IW10)
	if (sunxi_ehci->usbc_no == HCI0_USBC_NO) {
		val = readl(sunxi_ehci->usb_ccmu_config + USB_GATE);
		val |= (SUNXI_CCMU_USBEHCI1_GATING_OFFSET | SUNXI_CCMU_USBEHCI1_RST_OFFSET);
		writel(val, sunxi_ehci->usb_ccmu_config + USB_GATE);

		val = readl(sunxi_ehci->usb_ccmu_config + USB1_CFG);
		val |= (SUNXI_CCMU_SCLK_GATING_USBPHY1_OFFSET
			| SUNXI_CCMU_USBPHY1_RST_OFFSET
			| SUNXI_CCMU_SCLK_GATING_OHCI1_OFFSET);
		writel(val, sunxi_ehci->usb_ccmu_config + USB1_CFG);

		/*phy reg, offset:0x10 bit3 set 0, enable siddq*/
		val = USBC_Readl(sunxi_ehci->usb_common_phy_config + SUNXI_HCI_PHY_CTRL);
		val &= ~(0x1 << SUNXI_HCI_PHY_CTRL_SIDDQ);
		USBC_Writel(val, sunxi_ehci->usb_common_phy_config + SUNXI_HCI_PHY_CTRL);
	}
#endif
...
static int sunxi_ehci_hcd_suspend(struct device *dev)
{
	...
	if (sunxi_ehci->wakeup_suspend == USB_STANDBY) {
		...
		val = USBC_Readl(sunxi_ehci->usb_vbase + SUNXI_USB_PMU_IRQ_ENABLE);
		val |= (0x1 << 31);
		USBC_Writel(val, sunxi_ehci->usb_vbase + SUNXI_USB_PMU_IRQ_ENABLE);
		...
		val = USBC_Readl(sunxi_ehci->prcm);
		val |= (0x1 << 3);
		USBC_Writel(val, sunxi_ehci->prcm);
#endif
	} else {
...
static int sunxi_ehci_hcd_resume(struct device *dev)
{
	...
	if (sunxi_ehci->wakeup_suspend == USB_STANDBY) {
		...
		/*phy reg, offset:0x10 bit4 bit5, set 1*/
		val = ehci_readl(ehci, &ehci->regs->command);
		val |= 0x30;
		ehci_writel(ehci, val, &ehci->regs->command);
