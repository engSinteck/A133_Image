...
#define MTP_SEND_FILE32              _IOW('M', 0, struct mtp_file_range32)
#define MTP_RECEIVE_FILE32           _IOW('M', 1, struct mtp_file_range32)
#define MTP_SEND_EVENT32             _IOW('M', 3, struct mtp_event32)
#define MTP_SEND_FILE_WITH_HEADER32  _IOW('M', 4, struct mtp_file_range32)
...
static long mtp_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == MTP_SEND_FILE32)
		cmd = MTP_SEND_FILE;
	else if (cmd == MTP_RECEIVE_FILE32)
		cmd = MTP_RECEIVE_FILE;
	else if (cmd == MTP_SEND_EVENT32)
		cmd = MTP_SEND_EVENT;
	else if (cmd == MTP_SEND_FILE_WITH_HEADER32)
