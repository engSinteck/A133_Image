	...
	struct disp_layer_config_data *data)
{
	...
	for (i = 0; i < vi_chn; i++) {
		num_layers_video_by_chn[i] =
			bsp_disp_feat_get_num_layers_by_chn(disp, i);
	}
...
int disp_al_layer_apply(unsigned int disp,
	struct disp_layer_config_data *data, unsigned int layer_num)
{
	...
	struct disp_manager_data *data)
{
	if (data->flag & MANAGER_ENABLE_DIRTY) {
		al_priv.output_cs[data->config.hwdev_index] = data->config.cs;
		...
		al_priv.disp_disp[data->config.hwdev_index] = disp;
	}
...
int disp_al_capture_apply(unsigned int disp,
...
int disp_al_lcd_cfg(u32 screen_id,
	struct disp_panel_para *panel,
	struct panel_extend_para *extend_panel)
{
	...
	al_priv.output_fps[screen_id] = panel->lcd_dclk_freq * 1000000
		/ panel->lcd_ht / panel->lcd_vt;
...
int disp_al_lcd_cfg_ext(u32 screen_id,
	struct panel_extend_para *extend_panel)
{
 ...
 */
int disp_al_lcd_query_irq(u32 screen_id,
enum __lcd_irq_id_t irq_id, struct disp_panel_para *panel)
{
	int ret = 0;
...
int disp_al_lcd_enable_irq(u32 screen_id,
	enum __lcd_irq_id_t irq_id, struct disp_panel_para *panel)
{
	int ret = 0;
...
int disp_al_lcd_disable_irq(u32 screen_id,
	enum __lcd_irq_id_t irq_id, struct disp_panel_para *panel)
{
	int ret = 0;
...
int disp_al_lcd_get_cur_line(u32 screen_id,
	struct disp_panel_para *panel)
{
	return tcon_get_cur_line(screen_id, 1);
}

int disp_al_lcd_get_start_delay(u32 screen_id,
	struct disp_panel_para *panel)
{
...
int disp_al_hdmi_cfg(u32 screen_id, struct disp_video_timings *video_info)
{
	...
	al_priv.output_fps[screen_id] = video_info->pixel_clk
		/ video_info->hor_total_time / video_info->ver_total_time
		* (video_info->b_interlace + 1) / (video_info->trd_mode + 1);
	al_priv.tcon_index[screen_id] = 1;
...
int disp_al_vdevice_cfg(u32 screen_id,
		...
		*/
		...
		for (chn = 0; chn < chn_num; chn++) {
			...
			for (layer_id = 0; layer_id < layer_num; layer_id++) {
				if (de_rtmx_get_lay_enabled(disp, chn,
					layer_id)) {
					logo_find = 1;
					break;
				}
		...
		if (logo_find) {
			...
			de_rtmx_get_lay_address(disp, chn, layer_id,
				al_priv.logo_info[disp].fb.addr);
...
int disp_al_get_display_size(unsigned int screen_id,
